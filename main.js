        ...
        try {
            switch (true) {
                ...
                case ['.metallic','.ice','.snow','.neon','.fire','.glitch','.matrix'].some(p => userMessage.startsWith(p)): {
                    ...
                    try {
                        const imgs = await require('mumaker').ephoto(`https://en.ephoto360.com/${type}-text-effect`, text);
                        if (imgs?.image) await sock.sendMessage(chatId,{image:{url:imgs.image},caption:`✅ ${type} effect`},{quoted:message});
                        else throw new Error('no img');
                    } catch { await sock.sendMessage(chatId,{text:`❌ Failed to generate ${type} effect.`},{quoted:message}); }
                    break;
                }
